...
pokemon_typing = db.Table('pokemon_typing',
    db.Column('pokemon_id', db.ForeignKey("pokemon.id"), primary_key=True),
    db.Column('pokemontype_id', db.ForeignKey("pokemon_types.id"), primary_key=True)
)
...
class Item(db.Model):
    ...
    pokemon_id = db.Column(ForeignKey("pokemon.id"), nullable=False)
    happiness = db.Column(Integer, nullable=False)
    image_url = db.Column(String(255), nullable=False)
    name = db.Column(String(255), nullable=False)
    price = db.Column(Integer, nullable=False)
    poke_items = relationship("Pokemon", back_populates="items_of_pokes")
...
class Pokemon(db.Model):
    ...
    name = db.Column(String(255), nullable=False)
    attack = db.Column(Integer, nullable=False)
    defense = db.Column(Integer, nullable=False)
    image_url = db.Column(String(255), nullable=False)
    type_ = db.Column(Integer, nullable=False)
    moves = db.Column(String(255), nullable=False)
    encounter_rate = db.Column(Float)
    catch_rate = db.Column(Float)
    captured = db.Column(Boolean)
    types = relationship(
        "PokemonType",
        secondary=pokemon_typing,
        back_populates="pokemon"
    )
    items_of_pokes = relationship("Item", back_populates="poke_items")
